...
function registrar_usuario(usuario, pass) {
    ...
    if (buscar_usuario_registrado(usuario.value.toUpperCase())) {
        ...
        saludo_usuario.innerText = `Bienvenido/a ${usuario.value}`;

        fn_registrado();         
        fn_sweet_bienvenido(usuario.value.toUpperCase());
...
function login_usuario(usuario, pass) {
    let usuario_existente = buscar_usuario_registrado(usuario.value.toUpperCase());
    ...
    if (usuario_existente) {
        ...
        usr_nombre_error.style.display = "block";
        usr_nombre_error.innerText = usr_no_registrado;    
        return false;
    }
}

// Funcion que hace un Delay de x seg
const delay = ms => new Promise(res => setTimeout(res,ms));
const fn_delay = async () => {
    await delay(3000);
    console.log("3000");
    return true;    
};
